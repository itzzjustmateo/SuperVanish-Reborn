...
import de.devflare.svreborn.SuperVanishPlugin;
...
public class FileMgr {
    ...
    public PluginFile<?> addFile(String name, FileType type) {
        ...
        if (type == FileType.STORAGE) {
            StorageFile file = new StorageFile(name, (SuperVanishReborn) plugin);
            files.put(name, file);
            return file;
        } else if (type == FileType.CONFIG) {
            ConfigurableFile file = new ConfigurableFile(name, (SuperVanishReborn) plugin);
            files.put(name, file);
            return file;
        } else {
